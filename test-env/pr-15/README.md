...
* **💡 Progressive Logical Hint Engine:** A full, newbie-to-expert curriculum of solving techniques, each with the same 3-tier nudge (name the technique and point at the cell → explain the pattern without giving away the number → the exact digit and placement), applied in the order a person would actually try them:
  ...
  2. **Naked Pairs**, **Naked Triples**, **Pointing Pairs/Triples**, **Claiming Pairs/Triples** (Box-Line Reduction), & **X-Wing**
  3. **XY-Wing**, **XYZ-Wing**, **Unique Rectangle (Type 1)**, & **Swordfish**
  4. **Fallback Reveal** — used only when a puzzle genuinely needs logic beyond all of the above.
* **📊 4 Difficulty Tiers, Calibrated by Technique (Not Just Given-Count):** Every generated puzzle is checked against the hint engine itself before it's served — not just "does it have a unique solution."
  * **Easy:** Solvable using only Full House, Naked Single, and Hidden Single.
  * **Medium / Hard:** Each is verified to actually *need* at least one technique from its own tier (tier 2 / tier 3 above) — never a puzzle that's secretly easier wearing a harder label.
  * **Expert:** Verified to resist every technique above — a genuine logical dead end, where the hint engine's fallback reveal is the only way forward.
